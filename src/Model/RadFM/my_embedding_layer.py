...
class MyEmbedding(nn.Module):
    ...
    def __init__(self,
        ...
        self.weight = nn.Parameter(
            torch.torch.randn((num_embeddings, embedding_dim)))
        self.figure_token_weight = nn.Parameter(torch.randn(
            (2, embedding_dim)))
        self.flag = 'Text'
        ...
        # decoder_layer = TransformerDecoderLayer(d_model=vis_dim,
        #                                         nhead=8,
    ...
    def forward(self, text_input, vision_x, key_words_query=None):
        if self.flag == 'Text':
            ...
            #     query_words = [
            #         item for sublist in key_words_query for item in sublist
            #     ]
            ...
            #         contrastive_labels = contrastive_labels.to(
            #             vision_x.dtype).to(vision_x.device)
            ...
            #                 input_ids=query_words_embedding['input_ids'].to(
            #                     vision_x.device),
            #                 attention_mask=query_words_embedding[
            #                     'attention_mask'].to(vision_x.device)
            #             )['last_hidden_state'][:, 0, :].to(vision_x.dtype).to(
            #                 vision_x.device)  # Q,D
            #         query_words_embedding = self.bert_projection_fc(
            #             query_words_embedding)
            #         query_words_embedding = query_words_embedding.unsqueeze(
            #             0).repeat(B, 1, 1)  # B,Q,D
            #         _, N, _ = query_words_embedding.shape

            #         image_embedding = vision_x.mean(
            #             dim=1)  # B V D average pooling 去除掉多模态。
            #         image_embedding = rearrange(image_embedding,
            #                                     "b F v d -> b (F v) d")
            #         pos_embedding = rearrange(pos_embedding,
            #                                   "(b s) v d -> b s v d",
            #                                   b=B,
            #                                   s=S)[:, 0, :, :]

            #         image_embedding = image_embedding.transpose(
            #             0, 1)  # (H/P W/P D/P) B  D
            #         pos_embedding = pos_embedding.transpose(
            #             0, 1)  # (H/P W/P D/P) B  D
            #         query_words_embedding = query_words_embedding.transpose(
            #             0, 1)  # N B D
            ...
            embedding_weight = torch.cat(
                [self.weight, self.figure_token_weight], dim=0)
